...
ReadIHSpinY;
ReadIHSpinPhase;
ReadIsolatedHorizonSpin::usage = "ReadIsolatedHorizonSpin[run, idx, dir] returns the spin of horizon with index idx in run in the direction dir (counted from 1) as a DataTable.  This requires output from the QuasiLocalMeasures or IsolatedHorizon thorn to be present in the run directory.  dir is optional, and if omitted the result will be a vector-valued DataTable.";
ReadIsolatedHorizonDimensionlessSpin;
ReadIsolatedHorizonSpinPhase;
ReadAHMass::usage = "ReadAHMass[run, idx] returns the irreducible mass of apparent horizon with index idx in run.  This requires output from the AHFinderDirect thorn.";
ReadAHRadius::usage = "ReadAHRadius[run, idx] returns the coordinate radius of apparent horizon with index idx in run.  This requires output from the AHFinderDirect thorn.";
ReadAHMinRadius;
ReadAHMaxRadius;
ReadAHCentroid::usage = "ReadAHCentroid[run, idx] returns the coordinate centroid of apparent horizon with index idx in run.  This requires output from the AHFinderDirect thorn.";
ReadAHCentroidCoord;
ReadAHColumn;
ReadAHColumns;
ReadAHQuadrupoleXX;
...
ReadIsolatedHorizonDimensionlessSpin[runName_, hn_] :=
  ReadIsolatedHorizonSpin[runName, hn] ~DivideDataTables~ ChristodoulouMass[runName, hn+1, hn]^2;
