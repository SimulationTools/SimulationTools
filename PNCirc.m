...
SolveOrbitxDotOfx::usage = "SolveOrbitxDotOfx[inits, {t1, t2}] gives a PN orbit constructed using TaylorT4";
SpinWeightedSphericalHarmonic::usage = "SpinWeightedSphericalHarmonic[l,m,th,ph] gives the spin-weighted spherical harmonic";
GenerateStrain22::usage = "GenerateStrain[] computes the l = 2, m = 2 mode of the gravitational wave strain from an orbital solution";
...
GenerateStrain22[orbit_List, defs_List, {t1_, t2_}] :=
  Module[{t, phiFn, xFn, strain, h22Expr},
    {phiFn, xFn} = {phi,x} /.orbit;
    (* Here we drop the high order psi terms and just use phi *)
    h22Expr = Express[h22,x] /. psi -> phi /. mu -> Express[mu,nu] /. defs;
    strain = Function[t, Evaluate[Format[t] = Style["t",Bold]; Normal[h22Expr] /. {phi -> phiFn[t], x -> xFn[t]}]];
    Return[strain]];

(* We have the orbital solution as interpolating functions.  Things we could do:

   * Get analytic expressions for the various waveform quantities in
     terms of x and phi
...
derivRules[f_, t_] := {D[f[t],t] -> dot[f], D[f[t],t,t] -> dot[dot[f]],D[f[t],t,t,t] -> dot[dot[dot[f]]], f[t] -> f}

psi422Expr = Simplify[D[Express[h22, x] /.{x->x[t],psi->psi[t]},t,t]];
psi422FreqExpr = Simplify[Im[D[psi422Expr,t] / psi422Expr]];

Express[psi422, x] = psi422Expr /. derivRules[x,t] /. derivRules[psi,t];
Express[psi422Freq, x] = psi422FreqExpr /. derivRules[x,t] /. derivRules[psi,t];
